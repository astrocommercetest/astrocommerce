...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (recipientEmail) {
    ...
    const emailId = await sendEmail({ to: recipientEmail, subject: `Conferma ordine #${orderId.slice(0, 8).toUpperCase()}`, text: `Grazie per il tuo ordine!\n\nOrdine: #${orderId.slice(0, 8).toUpperCase()}\n\nArticoli:\n${itemLines}\n\nTotale: €${subtotal.toFixed(2)}\n\nSpedizione a:\n${shipping.name}\n${shipping.address}\n${shipping.zip} ${shipping.city} (${shipping.province})\nTel: ${shipping.phone}${shipping.notes ? `\nNote: ${shipping.notes}` : ""}` });
    if (emailId) await db.update(Orders).set({ emailId }).where(eq(Orders.id, orderId));
  }
